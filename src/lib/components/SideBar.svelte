<script lang="ts">
	...
	$: isMainLink = (menu: any) => {
		...
		return menu.urlRestrict === activeUrl.pathname;
	};
...
<div class="py-4 text-gray-500 dark:text-gray-400">
	...
	{/if}
	<ul class="mt-6">
		<!-- making an index for a menu and use it in below data -->
		{#each data.navbar as menu, a}
			<li class="relative px-6 py-3">
				...
				{#if menu.SubMenu == 0}
					<!-- check this thing for now about index compare is right -->
					<!-- <h2>id : {a}</h2>
					<h2>url : {menu.urlRestrict}</h2>
					<h2>idurl : {data.semuaArr[a]}</h2> -->

					{#if !(menu.urlRestrict && data.semuaArr[a] == null)}
						<a
